(()=>{
  ...
  async function render(){
    ...
    try{
      ...
      c.innerHTML=(players||[]).map((p,i)=>{const pr=pm.get(p.user_id);return `<button type="button" class="lf-row" data-lf-user="${esc(p.user_id)}"><span class="lf-rank">#${i+1}</span><span class="lf-identity">${avatar({display_name:p.display_name,avatar_url:pr?.avatar_url})}<span class="lf-copy"><strong>${esc(p.display_name||'Battle-brother')}</strong>${pr?.bio?`<small>${esc(pr.bio)}</small>`:''}</span></span><span class="lf-faction">${esc(p.primary_faction||'Unassigned')}</span><strong>${p.wins||0}W / ${p.losses||0}L / ${p.draws||0}D</strong><em>${pct(p)}%</em></button>`}).join('')||'<div class="empty">No registered players yet.</div>';
  ...
  async function openDossier(userId){
    ...
    const rows=(games||[]).map(g=>{
      const owner=g.owner_user_id===userId;
      const me=p.display_name||'Player';
      const opp=owner?(g.opponent_name||names.get(g.opponent_user_id)||'Unknown'):(names.get(g.owner_user_id)||'Unknown');
      const r=String(g.result||'draw').toLowerCase();
      const mine=owner?r:(r==='win'?'loss':r==='loss'?'win':'draw');
      const myVp=owner?g.player_score:g.opponent_score;
      const oppVp=owner?g.opponent_score:g.player_score;
      const winner=mine==='win'?me:mine==='loss'?opp:'DRAW';
      const myFaction=owner?g.player_faction:g.opponent_faction;
      const oppFaction=owner?g.opponent_faction:g.player_faction;
