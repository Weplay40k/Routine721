(()=>{
  ...
  let rendering=false;
  function currentTitle(){return (document.getElementById('pageTitle')?.textContent||'').trim().toLowerCase()}
  function isLeaderboard(){return currentTitle()==='leaderboard'}
  function container(){return document.getElementById('page')}
  function desiredPage(){return String(window.__lfDesiredPage||'').toLowerCase()}
  function shouldRender(){return isLeaderboard() && (!desiredPage() || desiredPage()==='leaderboard')}
  async function render(){
    if(!shouldRender()) return;
    const c=container(); if(!c||rendering)return; rendering=true;
    try{
    ...
    }catch(e){console.error('Leaderboard:',e);if(shouldRender())c.innerHTML=`<div class="empty">Could not load leaderboard: ${esc(e.message)}</div>`}
  ...
  async function openDossier(userId){
    ...
    const [{data:people},{data:playerRows}]=await Promise.all([
      ids.length?client.from('profiles').select('id,display_name').in('id',ids):Promise.resolve({data:[]}),
      ids.length?client.from('players').select('user_id,display_name').in('user_id',ids):Promise.resolve({data:[]})
    ]);
    const names=new Map();
    (playerRows||[]).forEach(x=>{if(x.user_id&&x.display_name)names.set(x.user_id,x.display_name)});
    (people||[]).forEach(x=>{if(x.id&&x.display_name)names.set(x.id,x.display_name)});
    names.set(userId,p.display_name||pr?.display_name||'Player');
    const rows=(games||[]).map(g=>{
      const ownerId=g.owner_user_id;
      const opponentId=g.opponent_user_id;
      const ownerName=names.get(ownerId)||g.owner_name||'Unknown';
      const opponentName=names.get(opponentId)||g.opponent_name||'Unknown';
      const selectedIsOwner=ownerId===userId;
      const me=selectedIsOwner?ownerName:opponentName;
      const opp=selectedIsOwner?opponentName:ownerName;
      const r=String(g.result||'draw').toLowerCase();
      const mine=selectedIsOwner?r:(r==='win'?'loss':r==='loss'?'win':'draw');
      const myVp=selectedIsOwner?g.player_score:g.opponent_score;
      const oppVp=selectedIsOwner?g.opponent_score:g.player_score;
      const winner=mine==='win'?me:mine==='loss'?opp:'DRAW';
      const myFaction=selectedIsOwner?g.player_faction:g.opponent_faction;
      const oppFaction=selectedIsOwner?g.opponent_faction:g.player_faction;
      return `<div class="lf-game"><div><strong>${esc(me)} VS ${esc(opp)}</strong><span>${esc(myFaction||'Unknown')} vs ${esc(oppFaction||'Unknown')} · ${date(g.played_at)}</span></div><div><b class="lf-result ${esc(mine)}">${esc(mine.toUpperCase())}</b><span>WINNER: ${esc(winner)} · ${myVp??'—'}–${oppVp??'—'} VP</span></div></div>`;
    }).join('');
  ...
  function schedule(){clearTimeout(window.__lfTimer);window.__lfTimer=setTimeout(()=>{if(shouldRender())render()},350)}
  document.addEventListener('click',e=>{
    const nav=e.target.closest('[data-view],[data-page],.nav-button');
    if(nav){const page=(nav.dataset.view||nav.dataset.page||'').toLowerCase();window.__lfDesiredPage=page;clearTimeout(window.__lfTimer);if(page==='leaderboard')setTimeout(()=>{window.__lfDesiredPage='leaderboard';render()},500);return}
    const row=e.target.closest('.lf-row');if(row){e.preventDefault();e.stopPropagation();openDossier(row.dataset.lfUser)}
  },true);
  new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});
  window.__lfDesiredPage=currentTitle()||'';
