(()=>{
  ...
  function schedule(){
    ...
    window.__lfTimer=setTimeout(()=>{if(shouldRender()) render()},350);
  }
  document.addEventListener('click',e=>{
    const nav=e.target.closest('[data-view],[data-page],.nav-button');
    if(nav){
      const page=(nav.dataset.view||nav.dataset.page||'').toLowerCase();
      window.__lfDesiredPage=page;
      clearTimeout(window.__lfTimer);
      if(page==='leaderboard') setTimeout(()=>{window.__lfDesiredPage='leaderboard';render()},500);
      return;
    }
    const row=e.target.closest('.lf-row');
    if(row){e.preventDefault();e.stopPropagation();openDossier(row.dataset.lfUser)}
  },true);
