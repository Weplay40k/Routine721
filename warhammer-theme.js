...
(function () {
    ...
    function installLeaderboardRanking() {
        ...
        window.loadLeaderboard = async function () {
            ...
            const { data: profiles, error } = await supabaseClient.from("profiles").select("id, display_name, avatar_url, main_army, main_faction").order("display_name", { ascending: true });
            ...
            const card = (item, index, qualified) => {
                ...
                const requirement = qualified ? `${s.played} GAMES — QUALIFIED` : `${s.played}/${MIN_LEADERBOARD_GAMES} GAMES — NEED ${Math.max(0, MIN_LEADERBOARD_GAMES - s.played)} MORE`;
                const allegiance = player.main_faction || getCategoryFromFaction(player.main_army);
                return `<div class="player-card ${qualified ? "r721-ranked-card" : "r721-unranked-card"}" data-leader-player="${escapeHtml(player.id)}"><div class="player-card-top">${avatarHtml(player)}<div><div class="player-card-name">${title}&nbsp; ${escapeHtml(player.display_name || "Battle-brother")}</div><div class="player-card-army">${escapeHtml(player.main_army || "NO MAIN ARMY")}</div>${allegiance ? `<div class="r721-player-allegiance"><span>FACTION ALLEGIANCE</span><strong>${escapeHtml(allegiance)}</strong></div>` : ""}</div></div><div class="player-stats-mini"><div class="mini-stat"><div class="mini-stat-number">${s.played}</div><div class="mini-stat-label">GAMES</div></div><div class="mini-stat"><div class="mini-stat-number">${s.wins}</div><div class="mini-stat-label">WINS</div></div><div class="mini-stat"><div class="mini-stat-number">${s.winRate}%</div><div class="mini-stat-label">WIN RATE</div></div></div><div class="r721-ranking-status">${requirement}</div></div>`;
            };
